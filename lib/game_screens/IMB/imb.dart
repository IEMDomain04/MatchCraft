...
import 'package:match_card_game/data/dataImb.dart';
import 'package:match_card_game/pages/game_winIMB.dart';
import 'package:match_card_game/pages/start_game_screen.dart';

class ImbGame extends StatefulWidget {
  const ImbGame({Key? key}) : super(key: key);

  @override
  State<ImbGame> createState() => _ImbGameState();
}

class _ImbGameState extends State<ImbGame> {
  ...
  void startGameAfterDelay() {
    Future.delayed(const Duration(seconds: 3), () {
      setState(() {
        ...
        _time = 60;
        startTimer(); // Start the 60-second countdown
      });
    });
  }
                                  ...
                                  onFlip: () {
                                    if (!_flip) {
                                      ...
                                      if (_previousIndex != index) {
                                        ...
                                        // Extract the last character of the image path to compare
                                        String prevChar = _data[_previousIndex]
                                            .substring(
                                                _data[_previousIndex].length -
                                                    6,
                                                _data[_previousIndex].length -
                                                    5);
                                        String currentChar = _data[index]
                                            .substring(_data[index].length - 6,
                                                _data[index].length - 5);
